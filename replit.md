...
✓ **Clean Admin Interface & Article Visibility Control** (July 31, 2025)
  - **Draft Article Issue Fixed**: Resolved critical issue where draft articles were potentially visible in live Knowledge Base due to featured flag priority
  - **Streamlined Interface**: Moved publish/unpublish controls to dropdown menu for cleaner layout alongside archive and delete options
  - **Smart Tag Display**: Limited visible tags to first 4 with "+X more" indicator to prevent visual clutter and improve readability
  - **Dropdown Publishing**: Added Eye/EyeOff icons with "Publish Article" and "Unpublish Article" options in action dropdown
  - **Status-Based Filtering**: Confirmed only published articles appear in public Knowledge Base, drafts hidden from users
  - **One-Click Publishing**: Administrators can instantly toggle articles between draft and published status from dropdown menu
