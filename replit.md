...
- **Infinite Re-Render Bug Fix (August 13, 2025)**: Resolved critical "Maximum update depth exceeded" errors in commission tracker by fixing multiple cascading issues: disabled aggressive 15-second auto-refetch in pipeline queries (replaced with 30-second cache), fixed cascading query invalidations in UniversalNavbar by delaying cache operations, optimized auth hook to only invalidate user-specific queries instead of all queries, and temporarily disabled NavigationHistoryProvider. These changes eliminated the infinite re-render loops caused by excessive query invalidations and provided stable commission tracker functionality.
